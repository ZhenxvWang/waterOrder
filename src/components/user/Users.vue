<template>
  <div>
    ...
    <el-dialog title="Modify the user" :visible.sync="editShow" width="50%" center @close="editClose" >
      <!-- 内容主题区域 -->
...
export default {
  data() {
    ...
    var checkMobie = (rule,value,callback)=>{
      ...
      if(value==''){callback(new Error('Please enter your mobile phone number'));}
      else if(!regMobie.test(value)){callback(new Error('Please enter a valid mobile phone number'));}
      else callback();
    }
    ...
    var checkPassword =  (rule,value,callback)=>{
      ...
      else if(!regPa.test(value))callback(new Error('The password contains 6 to 16 digits and underscores (_)'));
      else callback();
    }

    var checkUsername =  (rule,value,callback)=>{
      const regPa = /^\w{4,16}$/
      if(value=='')callback(new Error('Please enter a user name'));
      else if(!regPa.test(value)){
        callback(new Error('The user name contains 4 to 16 digits and underscores (_)'));
      }else{
    ...
    return {
      ...
      //表单的验证规则对象
      editRules: {
        nike: [{required: true,message: 'Please enter a nickname', trigger: 'blur',}],
  ...
  methods: {
    //获取所有的用户信息
    getList() {
      axios.post('/api/order/admin/getUsers',this.queryInfo).then(res=>{
        this.loading = false;
    ...
    editCommit(formName){
      this.$refs[formName].validate(valid => {
        if(valid){
          //请求
          axios.post('/api/order/admin/updateUserBase',this.editUser).then(res=>{
            if(res.data==1){
    ...
    async userDelete(id) {
      const re = await this.$confirm('This operation will permanently delete the user. Do you want to continue?', 'prompt', {
      ...
      if(re !='confirm'){
        ...
        axios.get('/api/order/admin/deleteUser?uid='+id).then(res=>{
          if(res.data==1){
